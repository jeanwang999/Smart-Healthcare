...
current_data = {
    # 讓年齡更年輕，更分散
    'age': np.random.randint(35, 80, NUM_CURRENT_WARDS), 
    'gender': np.random.choice(['M', 'F'], NUM_CURRENT_WARDS, p=[0.5, 0.5]),
    # 確保絕大多數人沒有共病
    'comorbidity': np.random.choice([0, 1], NUM_CURRENT_WARDS, p=[0.65, 0.35]), # 65% 無共病
    
    # 讓生理數據更接近正常值
    'hr_avg': np.random.normal(75, 5, NUM_CURRENT_WARDS).astype(int), # 平均心率降至 75 (更穩定)
    'spo2_min': np.random.normal(93, 1, NUM_CURRENT_WARDS).clip(min=94, max=100).astype(int), # 平均血氧升至 97 (下限提高到 94)
    'temp_max': np.random.normal(36.8, 0.3, NUM_CURRENT_WARDS), # 平均體溫降至 36.8
    'bp_systolic_min': np.random.normal(110, 10, NUM_CURRENT_WARDS).clip(min=100, max=140).astype(int), # 平均血壓更穩定
}
...
# 2. 模擬動態趨勢
# 讓趨勢變化極小
df_current['hr_change_rate'] = np.random.uniform(0.05, 1.0, NUM_CURRENT_WARDS) # 變化率上限降至 0.5
df_current['spo2_trend'] = np.random.uniform(-0.3, 0.1, NUM_CURRENT_WARDS) # 血氧趨勢幾乎不變
